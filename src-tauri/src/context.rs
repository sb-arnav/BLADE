...
extern "system" {
    ...
    fn winapi_GetWindowTextW(hwnd: *mut std::ffi::c_void, lpstring: *mut u16, nmaxcount: i32) -> i32;

    #[link_name = "GetWindowThreadProcessId"]
    fn winapi_GetWindowThreadProcessId(hwnd: *mut std::ffi::c_void, lpdwprocessid: *mut u32) -> u32;

    #[link_name = "OpenProcess"]
    fn winapi_OpenProcess(dwdesiredaccess: u32, binherithandle: i32, dwprocessid: u32) -> *mut std::ffi::c_void;
...
pub fn infer_activity(ctx: &WindowContext) -> String {
    ...
    if app.contains("code") || app.contains("cursor") || app.contains("vim") || app.contains("neovim") {
        if title.contains(".rs") { return "Writing Rust code".to_string(); }
        if title.contains(".ts") || title.contains(".tsx") { return "Writing TypeScript code".to_string(); }
        if title.contains(".py") { return "Writing Python code".to_string(); }
        if title.contains(".go") { return "Writing Go code".to_string(); }
        return "Coding in an editor".to_string();
    }

    if app.contains("chrome") || app.contains("firefox") || app.contains("edge") || app.contains("brave") {
        if title.contains("github") { return "Browsing GitHub".to_string(); }
        if title.contains("stack overflow") || title.contains("stackoverflow") { return "Looking up a programming question".to_string(); }
        if title.contains("docs") || title.contains("documentation") { return "Reading documentation".to_string(); }
        return "Browsing the web".to_string();
    }

    if app.contains("terminal") || app.contains("powershell") || app.contains("cmd") || app.contains("wt") {
