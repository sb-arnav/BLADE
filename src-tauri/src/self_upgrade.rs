...
pub async fn auto_install(gap: &CapabilityGap) -> InstallResult {
    ...
    let output = crate::cmd_util::silent_tokio_cmd("cmd")
        .args(["/C", cmd])
        .output()
        .await;
    #[cfg(not(target_os = "windows"))]
    let output = crate::cmd_util::silent_tokio_cmd("sh")
        .args(["-c", cmd])
        .output()
        .await;
...
// Pentest mode (authorization gate, model routing, command surface) lives in
// `pentest.rs` — extracted 2026-04-27 per the v1.2 maturity audit. The
// pentest commands are user-facing security research surface, not part of
// the self-upgrade catalog → install loop. See `crate::pentest` for the
// full module.
