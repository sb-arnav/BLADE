...
async fn poll_tentacle(platform: &str) -> Vec<TentacleReport> {
    ...
    match platform {
        ...
        "github" => {
            ...
            // v1.6 narrowing — open-PR sweep across deep_scan-discovered repos
            // is cut. Per-repo PR observation can be reintroduced when the
            // setup-as-conversation flow (v2.0) lets the user register repos
            // explicitly.
            let _ = token;
        }
        ...
        "ci" => {
            ...
            // v1.6 narrowing — CI sweep was driven by deep_scan repo list. Cut.
            // setup-as-conversation (v2.0) will let the user register repos
            // explicitly so the CI watcher can be reintroduced.
            let _ = token;
        }
