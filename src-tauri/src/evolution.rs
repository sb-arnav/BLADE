...
static CATALOG: &[CatalogEntry] = &[
    ...
    CatalogEntry {
        ...
        triggers: &["finder", "nautilus", "Files", "Explorer", "file manager"],
        description: "Sandboxed filesystem access — read/write files, list directories, search content via MCP (safer than blade_bash for file operations)",
        required_token_hint: None,
        auto_install: true,
    },
    CatalogEntry {
        ...
        triggers: &[],
        description: "Persistent entity knowledge graph — stores facts, relationships, and context across conversations. Always useful.",
        required_token_hint: None,
        auto_install: true,
    },
    CatalogEntry {
        name: "Brave Search",
        package: "@anthropic-labs/mcp-server-brave-search",
        command: "npx",
        args: &["-y", "@anthropic-labs/mcp-server-brave-search"],
        triggers: &["Brave", "brave.com", "search"],
        description: "Web search via Brave Search API — find information, research topics, check facts",
        required_token_hint: Some("BRAVE_API_KEY (api.search.brave.com → API Keys — free tier available)"),
        auto_install: false,
    },
    CatalogEntry {
        name: "Tavily (Web Research)",
        package: "@anthropic-labs/mcp-server-tavily",
        command: "npx",
        args: &["-y", "@anthropic-labs/mcp-server-tavily"],
        triggers: &["research", "web search"],
        description: "Deep web research — search, extract, and summarize web content for research tasks",
        required_token_hint: Some("TAVILY_API_KEY (app.tavily.com — free 1000 searches/month)"),
        auto_install: false,
    },
    CatalogEntry {
        name: "Composio (1000+ Integrations)",
        package: "composio-mcp",
        command: "npx",
        args: &["-y", "composio-mcp"],
        triggers: &[],
        description: "Connect to 1000+ apps (Gmail, Slack, Notion, Google Drive, HubSpot, Salesforce, etc.) through a single MCP server. Free tier: 1000 actions/month.",
        required_token_hint: Some("COMPOSIO_API_KEY (app.composio.dev — free tier available)"),
        auto_install: false,
    },
];
