...
| 18 ✅ | **Chat → Cross-App Action** *(closed 2026-04-30 — code complete; cold-install demo deferred per operator API-key constraint)* | Chat-first reinterpretation per CONTEXT D-01..D-21: text-chat → intent classifier → 3-tier dispatch → consent → outbound write → ego post-processor. PTT/voice deferred to v1.3. | JARVIS-03..11 + JARVIS-12-rewritten (12 REQs; 01/02 deferred to v1.3) | Code complete: 4 modules (ego/intent_router/jarvis_dispatch/consent) + 3 outbound tentacles + WriteScope RAII + tokio::oneshot consent + JarvisPill + ConsentDialog. Static gates 31/31 + 87 unit tests + tsc + verify:emit-policy all green. Cold-install runtime demo deferred (operator lacks API keys). | 4 |
| 19 ⏸ | **Operator UAT Close** *(DEFERRED to v1.3 — chat-first pivot 2026-04-30; pure UAT phase, operator lacks bandwidth + API keys)* | 11 v1.1 carry-overs + HANDOFF-TO-MAC.md reconcile. | UAT-01..12 (12 REQs) | All 12 REQs carry forward to v1.3 dedicated UAT phase; UAT-12 (HANDOFF-TO-MAC reconcile) absorbed into Phase 20 CHANGELOG. v1.1 milestone-audit stays `tech_debt`. | 2 |
...
- [x] `18-08-PLAN.md` — Wave 2: gmail_outbound base64url + Gmail API body. Covers JARVIS-04. ✅ shipped 2026-04-30 (12 unit tests green; URL_SAFE_NO_PAD encoding locked; D-10 hard-fail + 401 Reconnect Gmail wording verbatim; OAuth refresh deferred to v1.3 per RESEARCH § Watch Out).
- [x] `18-09-PLAN.md` — Wave 3: jarvis_dispatch_action body — consent gate + WriteScope + 3-tier dispatch + D-17 LOCKED activity-log emission. Covers JARVIS-04/05/09/10. ✅ shipped 2026-04-30 (10 unit tests green; cross-module 20/20 green; D-17 outcome vocabulary pinned by include_str! self-assertion test; SharedMcpManager pattern parity with slack/gmail outbound; linear/calendar branches return None on purpose with `// Plan 18-14 Task 2 wires this branch` markers; NeedsPrompt → NoConsent Wave-3 short-circuit, Plan 14 replaces with tokio::oneshot await).
- [x] `18-10-PLAN.md` — Wave 4: commands.rs ego wrap at l.~1517 + reset_retry_for_turn at function entry + research/questions.md Q1 closure. Covers JARVIS-03/06/07/08/09/10. ✅ shipped 2026-04-30 (3 surgical insertions in commands.rs — function entry l.718 reset_retry_for_turn + Pitfall 3 inline doc at fast-streaming branch l.1166 + ego intercept wrap at l.1539 with @-pattern verdict bind + extract_actions(&final_content) rewrite; 18 ego unit tests green; cargo check clean (only pre-existing consent_check_at Plan 14 testability-seam warning); Q1 closed with D-20 verdict — browser-harness installs ALWAYS require explicit consent, adoption decision deferred to v1.3; zero deviations).
- [x] `18-11-PLAN.md` — Wave 4: frontend — 6 typed Tauri wrappers + JarvisPill + ConsentDialog + MessageList/ChatPanel wiring. Covers JARVIS-05/11. ✅ shipped 2026-04-30 (3 task commits: e962ed3 admin.ts +138 lines with 3 type aliases + 6 wrappers; 618cd4f JarvisPill.tsx + ConsentDialog.tsx + chat.css with T-18-03 plain-text preview render and zero ghost tokens; 67a3077 MessageList JARVIS_INTERCEPT subscriber + ChatPanel CONSENT_REQUEST subscriber + ConsentDialog mount + Wave-4 simplified handleDecide with FORWARD-POINTER for Plan 14 supersession; tsc + verify:all green; D-13 listen-surface lock honored).
...
- [x] `18-14-PLAN.md` — Wave 4: pipeline-wiring — args extraction in intent_router + Linear/Calendar concrete wiring in jarvis_dispatch + tokio::oneshot consent (request_consent + consent_respond) + commands.rs (intent, args) wiring + ConsentDialog/ChatPanel one-shot. Closes 4 plan-checker BLOCKERS. Covers JARVIS-04/05. ✅ shipped 2026-04-30 (4 task commits: 495b8fd args extraction with `(IntentClass, ArgsBag)` return + 6 new heuristic tests for Linear/Slack/GitHub/Gmail/Calendar — 16/16 green; c37216f Linear+Calendar concrete wiring replacing Plan 09 `None  // PLACEHOLDER` with auto_create_ticket + calendar_post_meeting_summary — 12/12 green; 2230333 tokio oneshot consent — request_consent async fn owns oneshot channel + consent_respond Tauri command + ConsentChoice enum, dispatcher AWAITS in-place, AllowOnce passes through without SQLite write, AllowAlways persists then falls through, 60s timeout returns Deny with PENDING cleanup — 5 new oneshot tests; 52a276f commands.rs background dispatch + frontend simplification — admin.ts consentRespond wrapper, payloads.ts action_kind field, ChatPanel.handleDecide reduced 25→5 lines with single consentRespond call, Plan 11 re-invoke handler GONE, 18-DEFERRAL.md records D-04 Step 2 path B). 4 deviations all Rule 1/3 (slack test message lacked service token; email truncation in marker helper; placeholder-test self-match via include_str!; Plan 09 emit_consent_request dead-code cleanup). All 4 BLOCKERS closed: B1 path B (re-invoke GONE), B2 (Allow once works), B3 (Linear+Calendar wired), B4 (real args extracted). W1 closed (action_kind preserved). Verification: cargo check clean; cargo test --lib 43/43 green; tsc clean; emit-policy clean.
