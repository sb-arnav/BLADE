...
| 22 ✅ | **Voyager loop closure** *(shipped 2026-05-01)* ← *load-bearing* | Wire `evolution.rs → autoskills.rs → tool_forge.rs` end-to-end. One reproducible gap (`youtube_transcript`) closed. The substrate-level differentiator. | VOYAGER-01..09 (9/9 ✓) | All 5 SCs green: (1) `youtube_transcript` fixture closes deterministically in <2s; (2) `verify:voyager-loop` gate green (count 32 → 33); (3) 4 ActivityStrip emit points wired (gap_detected at immune_system entry, skill_written after fs::write, skill_registered after DB insert + SKILL.md export, skill_used in record_tool_use); (4) divergence property test asserts manifest set difference non-empty across 2 isolated installs; (5) DB-insert-fail rollback removes orphan script + re-logs gap with `prior_attempt_failed=true`. 28 unit tests + 2 fixture/property tests added. | 1 |
| 23 ✅ | **Verifiable reward + OOD eval** *(shipped 2026-05-01)* | RLVR-style composite reward in production + steelman Arg 3 mitigation. | REWARD-01..07 (7/7 ✓) | All 5 SCs green: (1) `compose()` at reward.rs:119 with 3 penalty paths ×0.7/×0.7/×0.0 (D-23-02); single-owner commands.rs hook (grep count = 1); (2) 9-field `RewardRecord` jsonl-appended atomically; (3) 52 hand-curated ASCII fixtures across `adversarial_eval` (17, floor 0.85) + `ambiguous_intent_eval` (18, floor 0.80) + `capability_gap_stress_eval` (17, floor 0.75); `verify-eval.sh` `EXPECTED=5→8` exits 0 with 8/8 tables; (4) `ood_gate_zeros_reward_on_15pct_drop` + `bootstrap_window_suppresses_gate` green; (5) `SignalClass::RewardTrend` + 3 verbatim D-23-04 suggested_fix arms + 6-arm tokio::join + TS lockstep across payloads.ts/admin.ts/DoctorPane.tsx (UAT-deferred per chat-first pivot). 33+ unit tests added; verify chain stays at 33 (gate extension). | 2 |
