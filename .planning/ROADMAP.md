...
|---|---|---|---|---|---|
| 21 | **Skills v2 / agentskills.io adoption** | The substrate prerequisite. SKILL.md (YAML+MD) format, progressive disclosure, workspace→user→bundled resolution, validator + 3 bundled exemplars. | SKILLS-01..08 (8 REQs) | (1) `blade skill validate` green on 3 bundled exemplars; (2) progressive-disclosure token-budget assertion holds (body bytes loaded only on activation); (3) workspace path wins over user path on name collision; (4) `verify:skill-format` gate landed in `verify:all` chain (count 31 → 32) | 2 |
