...
// ─── security_monitor.rs types ───────────────────────────────────────────────

/** @see src-tauri/src/security_monitor.rs:15 NetworkConnection */
...
/** @see src-tauri/src/security_monitor.rs:25 BreachResult */
...
/** @see src-tauri/src/security_monitor.rs:41 SensitiveFile */
...
/** @see src-tauri/src/security_monitor.rs:50 UrlSafetyResult */
...
/** @see src-tauri/src/security_monitor.rs:59 SecurityOverview */
export interface SecurityOverview {
  network_total: number;
  network_suspicious: number;
  files_found: number;
  files_unprotected: number;
  last_scan_ts: number;
  summary: string;
  [k: string]: unknown;
}

/** @see src-tauri/src/security_monitor.rs:1131 SecurityReport */
export interface SecurityReport {
  scope: string;
  started_at: number;
  finished_at: number;
  recon_findings: string;
  analysis_findings: string;
  report: string;
  [k: string]: unknown;
}

/** @see src-tauri/src/security_monitor.rs:1147 DepVulnerability */
...
/** @see src-tauri/src/security_monitor.rs:1161 SecurityIssue */
...
// ═══════════════════════════════════════════════════════════════════════════
// security_monitor.rs — security fortress (9 commands)
// ═══════════════════════════════════════════════════════════════════════════

/**
 * @see src-tauri/src/security_monitor.rs:903 security_scan_network
 * Rust signature: `security_scan_network() -> Result<Vec<NetworkConnection>, String>`.
 */
export function securityScanNetwork(): Promise<NetworkConnection[]> {
  return invokeTyped<NetworkConnection[]>('security_scan_network');
}

/**
 * @see src-tauri/src/security_monitor.rs:908 security_check_breach
 * Rust signature: `security_check_breach(email: String) -> Result<BreachResult, String>`.
 */
export function securityCheckBreach(email: string): Promise<BreachResult> {
  return invokeTyped<BreachResult, { email: string }>('security_check_breach', { email });
}

/**
 * @see src-tauri/src/security_monitor.rs:913 security_check_password_hash
 * Rust signature: `security_check_password_hash(hash: String) -> Result<bool, String>`.
 */
export function securityCheckPasswordHash(hash: string): Promise<boolean> {
  return invokeTyped<boolean, { hash: string }>('security_check_password_hash', { hash });
}

/**
 * @see src-tauri/src/security_monitor.rs:918 security_scan_sensitive_files
 * Rust signature: `security_scan_sensitive_files() -> Vec<SensitiveFile>`.
 */
export function securityScanSensitiveFiles(): Promise<SensitiveFile[]> {
  return invokeTyped<SensitiveFile[]>('security_scan_sensitive_files');
}

/**
 * @see src-tauri/src/security_monitor.rs:923 security_check_url
 * Rust signature: `security_check_url(url: String) -> UrlSafetyResult`.
 */
export function securityCheckUrl(url: string): Promise<UrlSafetyResult> {
  return invokeTyped<UrlSafetyResult, { url: string }>('security_check_url', { url });
}

/**
 * @see src-tauri/src/security_monitor.rs:928 security_overview
 * Rust signature: `security_overview() -> SecurityOverview`.
 */
...
/**
 * @see src-tauri/src/security_monitor.rs:1702 security_run_audit
 * Rust signature: `security_run_audit(scope: String) -> Result<SecurityReport, String>`.
 */
export function securityRunAudit(scope: string): Promise<SecurityReport> {
  return invokeTyped<SecurityReport, { scope: string }>('security_run_audit', { scope });
}

/**
 * @see src-tauri/src/security_monitor.rs:1708 security_audit_deps
 * Rust signature: `security_audit_deps(project_path: String) -> Result<Vec<DepVulnerability>, String>`.
 */
export function securityAuditDeps(projectPath: string): Promise<DepVulnerability[]> {
  return invokeTyped<DepVulnerability[], { project_path: string }>('security_audit_deps', {
    project_path: projectPath,
  });
}

/**
 * @see src-tauri/src/security_monitor.rs:1714 security_scan_code
 * Rust signature: `security_scan_code(file_path: String) -> Result<Vec<SecurityIssue>, String>`.
 */
export function securityScanCode(filePath: string): Promise<SecurityIssue[]> {
  return invokeTyped<SecurityIssue[], { file_path: string }>('security_scan_code', {
    file_path: filePath,
  });
}
