...
const DOC_TYPE_COLORS: Record<string, string> = {
  ...
  txt: "bg-[rgba(255,255,255,0.07)]/60 text-[rgba(255,255,255,0.7)] border-[rgba(255,255,255,0.15)]",
...
function ConfidenceBar({ value }: { value: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-xs text-[rgba(255,255,255,0.5)] w-8 text-right">{pct}%</span>
    </div>
  );
}
    ...
    <div className="border border-[rgba(255,255,255,0.1)] rounded overflow-hidden">
      ...
      >
        <svg
          ...
          className={`text-[rgba(255,255,255,0.4)] transition-transform ${open ? "rotate-90" : ""}`}
        >
...
export function DocumentView({ onBack }: { onBack: () => void }) {
  ...
  const docTypeStyle = (type: string) => {
    ...
    return DOC_TYPE_COLORS[key] || "bg-[rgba(255,255,255,0.07)]/60 text-[rgba(255,255,255,0.7)] border-[rgba(255,255,255,0.15)]";
  };
  ...
  return (
    <div className="flex flex-col h-full bg-black text-[rgba(255,255,255,0.85)] font-mono">
      ...
      <div className="flex items-center gap-3 px-4 py-2 border-b border-[rgba(255,255,255,0.1)] bg-black">
        ...
        <span className="text-xs text-[rgba(255,255,255,0.3)]">{docs.length} docs</span>
      </div>
        ...
        <div className="w-64 border-r border-[rgba(255,255,255,0.1)] flex flex-col overflow-hidden bg-[#09090b]/30">
          ...
          <div className="flex-1 overflow-y-auto">
            {filteredDocs.length === 0 && (
              <p className="text-xs text-[rgba(255,255,255,0.3)] text-center p-4">No documents yet</p>
            )}
            {filteredDocs.map((d) => (
              <button
                key={d.id}
                onClick={() => { setSelectedDoc(d); setActiveTab("detail"); setStudyNotes([]); }}
                className={`w-full text-left px-3 py-2.5 border-b border-[rgba(255,255,255,0.07)] hover:bg-[rgba(255,255,255,0.04)]/50 transition-colors ${selectedDoc?.id === d.id ? "bg-[rgba(255,255,255,0.04)]/60 border-l-2 border-l-green-600" : ""}`}
              >
                <div className="flex items-start justify-between gap-1 mb-1">
                  <p className="text-xs text-white font-semibold leading-tight truncate flex-1">{d.title}</p>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex border-b border-[rgba(255,255,255,0.1)] bg-black px-4">
            {(["detail", "qa", "synthesis"] as const).map((t) => (
              <button
                ...
                className={`px-4 py-2 text-xs uppercase tracking-wider border-b-2 transition-colors ${activeTab === t ? "border-green-400 text-green-400" : "border-transparent text-[rgba(255,255,255,0.4)] hover:text-[rgba(255,255,255,0.7)]"}`}
              >
          ...
          {activeTab === "detail" && (
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {!selectedDoc ? (
                <div className="flex items-center justify-center h-64 text-[rgba(255,255,255,0.3)] text-sm">
                ...
                <>
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                          ...
                          <span className="text-xs text-[rgba(255,255,255,0.4)]">{selectedDoc.word_count.toLocaleString()} words</span>
                          <span className="text-xs text-[rgba(255,255,255,0.3)]">{formatDate(selectedDoc.created_at)}</span>
                        </div>
                      </div>
                      {deleteConfirm === selectedDoc.id ? (
                        <div className="flex gap-1">
                          ...
                          <button
                            ...
                            className="px-2 py-1 text-xs text-[rgba(255,255,255,0.4)] border border-[rgba(255,255,255,0.1)] rounded"
                          >
                            Cancel
                          </button>
                        </div>
                        ...
                        <button
                          ...
                          className="text-xs text-[rgba(255,255,255,0.3)] hover:text-red-400 transition-colors border border-[rgba(255,255,255,0.1)] px-2 py-1 rounded"
                        >
                          Delete
                        </button>
                      )}
                  ...
                  {selectedDoc.topics.length > 0 && (
                    <div className="flex flex-wrap gap-1.5">
                      {selectedDoc.topics.map((t) => (
                        <span key={t} className="text-xs px-2 py-0.5 bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.1)] text-[rgba(255,255,255,0.7)] rounded-full">
                          {t}
                        </span>
                      ))}
                    </div>
                  ...
                  {selectedDoc.summary && (
                    <div className="border border-[rgba(255,255,255,0.1)] rounded bg-[#09090b]/30">
                      ...
                      >
                        <svg
                          width="10" height="10" viewBox="0 0 10 10" fill="currentColor"
                          className={`text-[rgba(255,255,255,0.4)] transition-transform ${summaryOpen ? "rotate-90" : ""}`}
                        >
                          <path d="M3 2l4 3-4 3" stroke="currentColor" strokeWidth="1.2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                        <span className="text-xs text-[rgba(255,255,255,0.5)] uppercase tracking-wider">Summary</span>
                      </button>
                      {summaryOpen && (
                        <div className="px-4 pb-4 border-t border-[rgba(255,255,255,0.07)]">
                          <p className="text-xs text-[rgba(255,255,255,0.7)] leading-relaxed mt-3 whitespace-pre-wrap">{selectedDoc.summary}</p>
                        </div>
                      )}
                    </div>
                  )}
                    ...
                    <div className="border border-[rgba(255,255,255,0.1)] rounded bg-[#09090b]/30 p-4">
                      ...
                      <ul className="space-y-1.5">
                        {selectedDoc.key_points.map((pt, i) => (
                          <li key={i} className="flex items-start gap-2 text-xs text-[rgba(255,255,255,0.7)]">
                  ...
                  <div className="border border-[rgba(255,255,255,0.1)] rounded bg-[#09090b]/30 p-4 space-y-3">
                    ...
                    {studyNotes.length > 0 ? (
                      ...
                      <p className="text-xs text-[rgba(255,255,255,0.3)] italic">Generate Q&A study notes from this document.</p>
                    ) : (
              ...
              <div className="space-y-3 border border-[rgba(255,255,255,0.1)] rounded bg-[#09090b]/30 p-4">
                ...
                <div className="space-y-2">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-xs text-[rgba(255,255,255,0.7)]">Ask All Docs ({docs.length})</span>
                  </label>

                  {!askAllDocs && (
                    <div className="max-h-32 overflow-y-auto border border-[rgba(255,255,255,0.1)] rounded p-2 space-y-1">
                      {docs.map((d) => (
                        <label key={d.id} className="flex items-center gap-2 cursor-pointer">
                          <input
                            ...
                            className="accent-green-500"
                          />
                          <span className="text-xs text-[rgba(255,255,255,0.7)] truncate">{d.title}</span>
                        </label>
                      ))}
                    </div>
                  )}
              ...
              {qaResult && (
                <div className="space-y-3">
                  ...
                  <div className="border border-green-800 rounded bg-green-900/10 p-4">
                    ...
                    <p className="text-sm text-white leading-relaxed whitespace-pre-wrap">{qaResult.answer}</p>
                    <div className="mt-3">
                      <p className="text-xs text-[rgba(255,255,255,0.4)] mb-1">Confidence</p>
                  ...
                  {qaResult.docs_used.length > 0 && (
                    <div className="flex flex-wrap gap-1.5">
                      {qaResult.docs_used.map((d, i) => (
                        <span key={i} className="text-xs px-2 py-0.5 bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.1)] text-[rgba(255,255,255,0.5)] rounded">
                          {d}
                        </span>
                      ))}
                    </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              {synthResult && (
                <div className="border border-purple-800 rounded bg-purple-900/10 p-4">
                  ...
                  <p className="text-sm text-white leading-relaxed whitespace-pre-wrap">{synthResult}</p>
                </div>
              )}
