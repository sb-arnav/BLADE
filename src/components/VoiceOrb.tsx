...
const STATUS_CONFIG: Record<VoiceStatus, { color: string; label: string; pulse: boolean }> = {
  ...
  error:      { color: "bg-red-500/60",            label: "Mic error",     pulse: false },
};

export function VoiceOrb({ status, mode }: Props) {
  if (mode === "off" || mode === "push-to-talk" || status === "idle") return null;

  const cfg = STATUS_CONFIG[status];

  return (
    <div className="fixed bottom-20 right-4 z-50 flex items-center gap-2 pointer-events-none select-none">
